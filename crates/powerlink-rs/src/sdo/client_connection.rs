// crates/powerlink-rs/src/sdo/client_connection.rs
//! Defines the state machine and logic for a single SDO client connection.
//!
//! This module handles the details of SDO transfer states (Expedited vs Segmented),
//! timeout tracking, retransmissions, and the processing of specific jobs like
//! Concise DCF downloads.
...
/// Maximum payload size for an SDO segment sent by the client.
/// (1456 max SDO payload - 4 byte command header = 1452)
const MAX_CLIENT_PAYLOAD: usize = 1452;

/// Internal state of a single SDO client-side transfer.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum SdoClientConnectionState {
...
#[derive(Debug, Clone)]
pub(crate) struct SdoClientConnection {
...
impl SdoClientConnection {
    pub(crate) fn new(target_node_id: NodeId) -> Self {
        Self {
            target_node_id,
    ...
    pub(crate) fn handle_response(&mut self, seq_header: &SequenceLayerHeader, cmd: &SdoCommand) {
    ...
    pub(crate) fn start_concise_dcf_job(
        &mut self,
        dcf_data: Vec<u8>,
        tid: u8,
        current_time_us: u64,
