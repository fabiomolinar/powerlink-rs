...
/// The priority is:
/// 1. Pending StatusRequests due to error signaling (EN != EA).
/// 2. Pending NMT Commands generated by the MN.
/// 3. Pending generic async frames generated by the MN application.
/// 4. Find and identify unknown/missing nodes.
/// 5. Poll async-only nodes.
/// 6. Service pending ASnd requests from CNs.
pub(super) fn determine_next_async_action(node: &mut ManagingNode) -> (RequestedServiceId, NodeId) {
    // 1. Check for pending StatusRequests due to error signaling (highest priority).
    if let Some(node_to_query) = node.pending_status_requests.pop() {
        info!(
            "[MN] Prioritizing StatusRequest for Node {} due to error signal.",
            node_to_query.0
        );
        return (RequestedServiceId::StatusRequest, node_to_query);
    }

    // 2. Check for pending NMT commands generated by the MN.
    if !node.pending_nmt_commands.is_empty() {
    ...
    // 3. Check for pending generic async frames from the MN application.
    if !node.mn_async_send_queue.is_empty() {
    ...
    // 7. If nothing else to do, send a SoA with NoService
...
pub(super) fn find_next_async_only_to_poll(node: &mut ManagingNode) -> Option<NodeId> {
    ...
    for i in 0..node.async_only_nodes.len() {
        ...
        if let Some(info) = node.node_info.get(&node_id) {
...
pub(super) fn get_next_isochronous_node_to_poll(
    ...
    while node.next_isoch_node_idx < node.isochronous_nodes.len() {
