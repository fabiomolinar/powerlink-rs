...
/// The priority is:
/// 1. Pending NMT Commands generated by the MN.
/// 2. Pending generic async frames generated by the MN application.
/// 3. Find and identify unknown/missing nodes.
/// 4. Poll async-only nodes.
/// 5. Service pending ASnd requests from CNs.
pub(super) fn determine_next_async_action(node: &mut ManagingNode) -> (RequestedServiceId, NodeId) {
    // 1. Check for pending NMT commands generated by the MN (highest priority).
    ...
    // 2. Check for pending generic async frames from the MN application.
    ...
    // 6. If nothing else to do, send a SoA with NoService
