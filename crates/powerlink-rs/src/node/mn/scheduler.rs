...
pub(super) fn determine_next_async_action(
    ...
    if let Some(node_to_reset) = context.pending_er_requests.pop() {
        pl_info!(*context, "Prioritizing ER for Node {}.", node_to_reset.0);
    ...
    if let Some(node_to_query) = context.pending_status_requests.pop() {
        pl_info!(*context, 
            "Prioritizing StatusRequest for Node {} due to error signal.",
            node_to_query.0
        );
    ...
    if !context.pending_nmt_commands.is_empty() {
        pl_info!(*context, "Prioritizing internal NMT command for async slot.");
        return (
    ...
    if !context.mn_async_send_queue.is_empty() {
        pl_info!(*context, "Prioritizing MN-initiated generic async frame for async slot.");
        return (
            RequestedServiceId::UnspecifiedInvite,
            NodeId(C_ADR_MN_DEF_NODE_ID),
            false,
    ...
    {
        pl_info!(*context, 
            "Prioritizing MN-initiated SDO request to Node {} for async slot.",
            target_node_id.0
        );
    ...
    if context.mn_async_send_queue.is_empty() {
        if let Some(service_id) = context.publish_config.get(&context.current_multiplex_cycle) {
            pl_trace!(*context, 
                "Scheduling NMT Info Broadcast ({:?}) for async slot.",
                service_id
            );
    ...
    if let Some(request) = context.async_request_queue.pop() {
        pl_info!(*context, 
            "Granting async slot to Node {} (PR={})",
...
pub(super) fn check_bootup_state(context: &mut MnContext) {
    ...
    if current_mn_state == NmtState::NmtPreOperational1 {
        ...
        if validation::check_all_mandatory_identified(&context.mandatory_nodes, &context.node_info) {
            pl_info!(*context, "All mandatory nodes identified. Triggering NMT transition to PreOp2.");
            context
                .nmt_state_machine
            ...
            for (node_id, info) in context.node_info.iter() {
                if info.state == CnState::Identified {
                    pl_info!(*context, 
                        "BOOT_STEP2: Queuing NMTEnableReadyToOperate for Node {}.",
                        node_id.0
                    );
        ...
        if validation::check_all_mandatory_preop(&context.mandatory_nodes, &context.node_info) {
            if context.nmt_state_machine.startup_flags & (1 << 8) == 0 {
                pl_info!(*context, 
                    "All mandatory nodes PreOperational/ReadyToOp. Triggering NMT transition to ReadyToOp."
                );
                ...
                pl_debug!(*context, 
                    "All mandatory nodes PreOperational/ReadyToOp, but waiting for application trigger to enter ReadyToOp."
                );
            }
        }
        ...
        if validation::check_all_mandatory_comm_verified(
            ...
            if context.nmt_state_machine.startup_flags & (1 << 2) == 0 {
                pl_info!(*context, 
                    "CHECK_COMMUNICATION passed for all mandatory nodes. Triggering NMT transition to Operational."
                );
                context
                    .nmt_state_machine
                ...
                pl_debug!(*context, 
                    "All mandatory nodes passed CHECK_COMMUNICATION, but waiting for application trigger to enter Operational."
                );
            }
        }
    }
...
pub(super) fn find_next_node_to_identify(context: &mut MnContext) -> Option<NodeId> {
    ...
    loop {
        ...
        if current_node_id_val == start_node_id_val {
            if wrapped_around {
                pl_debug!(*context, "Full circle check for unidentified nodes completed.");
        ...
        if matches!(
            ...
            pl_debug!(*context, 
                "Found unidentified or missing Node {} to poll.",
                node_id.0
            );
    ...
    pl_debug!(*context, "No more unidentified nodes found.");
    None
}
    ...
    for i in 0..context.async_only_nodes.len() {
        ...
        if let Some(info) = context.node_info.get(&node_id) {
            ...
            if info.state != CnState::Missing {
                pl_debug!(*context, 
                    "Found async-only Node {} to poll for status.",
                    node_id.0
                );
    ...
    while context.next_isoch_node_idx < context.isochronous_nodes.len() {
        ...
        if should_poll_this_cycle {
            ...
            if state >= CnState::PreOperational && state != CnState::Stopped {
                ...
                pl_debug!(*context, 
                    "Skipping Node {} (State: {:?}) in isochronous polling for mux cycle {}.",
                    node_id.0, state, current_multiplex_cycle
                );
            }
        } else {
            pl_trace!(*context, 
                "Skipping Node {} (assigned mux cycle {}) in current mux cycle {}.",
