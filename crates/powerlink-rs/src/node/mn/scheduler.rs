...
/// The priority is:
/// 1. Pending Exception Reset (ER) requests.
/// 2. Pending StatusRequests due to error signaling (EN != EA).
/// 3. Pending NMT Commands generated by the MN.
/// 4. Pending generic async frames generated by the MN application.
/// 5. Pending SDO client requests from the MN.
/// 6. Find and identify unknown/missing nodes.
/// 7. Poll async-only nodes.
/// 8. Service pending ASnd requests from CNs.
pub(super) fn determine_next_async_action(
    ...
    if !node.pending_sdo_client_requests.is_empty() {
