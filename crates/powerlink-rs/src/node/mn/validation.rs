...
pub(super) fn validate_boot_step1_checks(
    ...
    if expected_device_type != 0 && received_device_type != expected_device_type {
        pl_error!(*context, 
            "CHECK_IDENTIFICATION failed Node {}: DeviceType mismatch.",
            node_id.0
        );
        return false;
    }
    if expected_vendor_id != 0 && received_vendor_id != expected_vendor_id {
        pl_error!(*context, 
            "CHECK_IDENTIFICATION failed Node {}: VendorId mismatch.",
            node_id.0
        );
        return false;
    }
    if expected_product_code != 0 && received_product_code != expected_product_code {
        pl_error!(*context, 
            "CHECK_IDENTIFICATION failed Node {}: ProductCode mismatch.",
            node_id.0
        );
        return false;
    }
    if expected_revision_no != 0 && received_revision_no != expected_revision_no {
        pl_error!(*context, 
            "CHECK_IDENTIFICATION failed Node {}: RevisionNo mismatch.",
            node_id.0
        );
        return false;
    }

    pl_trace!(*context, "CHECK_IDENTIFICATION passed for Node {}.", node_id.0);
    ...
    if (startup_flags & (1 << 10)) != 0 {
        ...
        if sw_update_required {
            pl_warn!(*context, 
                "CHECK_SOFTWARE failed for Node {}. Update required.",
                node_id.0
            );
        ...
        pl_trace!(*context, "CHECK_SOFTWARE passed for Node {}.", node_id.0);
    }
    ...
    if (startup_flags & (1 << 11)) != 0 {
        ...
        if config_mismatch {
            pl_warn!(*context, 
                "CHECK_CONFIGURATION failed for Node {}. Expected {}/{}, Got {}/{}.",
                node_id.0,
                expected_conf_date,
                expected_conf_time,
                received_conf_date,
            ...
            if let Some(cfg_if) = context.configuration_interface {
                ...
                match cfg_if.get_configuration(node_id.0) {
                    Ok(concise_dcf) => {
                        ...
                        if let Err(e) = context.sdo_client_manager.start_configuration_download(
                            ...
                            pl_error!(*context, "[MN-CFM] Failed to start configuration download: {:?}", e);
                        } else {
                ...
                pl_error!(*context, 
                    "Configuration mismatch, but no Configuration Interface provided to fix it."
                );
                return false;
            }
        }
        pl_trace!(*context, "CHECK_CONFIGURATION passed for Node {}.", node_id.0);
    }
