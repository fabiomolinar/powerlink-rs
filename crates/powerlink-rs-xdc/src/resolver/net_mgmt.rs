            ...
            list.error
                ...
                .map(|e| types::ErrorDefinition {
                    ...
                    add_info: e.add_info.iter().map(|ai| types::AddInfo {
                        ...
                        description: ai.labels.as_ref().and_then(|l| utils::extract_description(&l.items)), // FIX: Pass .items
                    }).collect(),
                })
                .collect()
        });
    ...
    let static_error_bit_field = model.static_error_bit_field.as_ref().map(|field| {
        field.error_bit.iter().map(|bit| {
            types::StaticErrorBit {
                ...
                label: bit.labels.as_ref().and_then(|l| utils::extract_label(&l.items)), // FIX: Pass .items
                description: bit.labels.as_ref().and_then(|l| utils::extract_description(&l.items)), // FIX: Pass .items
            }
        }).collect()
    });
...
mod tests {
    ...
    fn test_extract_label() {
        ...
        assert_eq!(extract_label(&glabels_empty.unwrap_or_default().items), None);
        ...
        assert_eq!(extract_label(&glabels_desc.unwrap_or_default().items), None);
        ...
        assert_eq!(
            extract_label(&glabels_one.unwrap_or_default().items),
        ...
        assert_eq!(
            extract_label(&glabels_multi.unwrap_or_default().items),
            Some("First Label".to_string())
        );
    }
    ...
    fn test_extract_description() {
        ...
        // Test Some(Glabels) with no items
        let glabels_empty = Some(Glabels { items: vec![] });
        assert_eq!(extract_description(&glabels_empty.unwrap_or_default().items), None);
        ...
        assert_eq!(extract_description(&glabels_label.unwrap_or_default().items), None);
        ...
        assert_eq!(
            extract_description(&glabels_one.unwrap_or_default().items),
        ...
        assert_eq!(
            extract_description(&glabels_multi.unwrap_or_default().items),
            Some("First Description".to_string())
        );
    }
