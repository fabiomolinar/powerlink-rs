...
pub(super) fn resolve_network_management(
    ...
    let general_features = types::GeneralFeatures {
        ...
        nmt_boot_time_not_active: parse_u32_attr(Some(
            model.general_features.nmt_boot_time_not_active.clone(),
        )),
    ...
    let cn_features = model.cn_features.as_ref().map(|cn| types::CnFeatures {
        ...
        nmt_cn_pre_op2_to_ready2_op: cn
...
mod tests {
    ...
    fn test_extract_label() {
        ...
        assert_eq!(
            extract_label(&glabels_empty.unwrap_or_default().items),
            None
        );
    ...
    fn test_extract_description() {
        ...
        let glabels_empty = Some(Glabels { items: vec![] });
        assert_eq!(
            extract_description(&glabels_empty.unwrap_or_default().items),
            None
        );
        ...
        assert_eq!(
            extract_description(&glabels_label.unwrap_or_default().items),
            None
        );
    ...
    fn test_resolve_network_management() {
        ...
        assert_eq!(cnf.nmt_cn_dna, Some(types::NmtCnDna::ClearOnPreOp1ToPreOp2));
    }
}
